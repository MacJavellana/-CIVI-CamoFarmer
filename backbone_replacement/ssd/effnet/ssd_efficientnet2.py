...
from backbone.backbones import EfficientNetB0Backbone

# helper that ignores the marker string 'S'
def get_extras(config, in_channels, batch_norm=False):
    ...
    for i, out_channels in enumerate(config):
        if out_channels == 'S' or isinstance(out_channels, tuple):
            ...
            if i + 1 < len(config):  # check if next channel exists
...
class SSDEfficientNet(nn.Module):
    """
    SSD architecture with EfficientNet-B0 backbone.
    Assumes the backbone returns 6 features maps with channels 40, 80, 80, 112, 192, 320.
    """
    def __init__(self, 
                 mode, 
                 backbone, 
                 ...
                 anchors, 
                 class_count):
        super(SSDEfficientNet, self).__init__()
    ...
    def forward(self, x):
        ...
        feats = self.backbone(x)  # [feat1, feat2, feat3, feat4, feat5]

        #  DEBUG: Print feature map shapes from EfficientNet backbone
        # for i, feature in enumerate(feats):
        #     print(f"Feature Map {i} Shape: {feature.shape}")  # <-- Debug print

        # check if all backbone features are used as sources
        sources.append(self.L2Norm(feats[0]))  # apply L2Norm to feat1
        # print("added l2norm to first feature map!")
        sources.extend(feats[1:])              # append the rest as-is
    ...
    def init_weights(self, model_save_path, basenet):
        if basenet:
            ...
            print(f"Using EfficientNet weights! ({weights_path})")

            efficientnet_weights = torch.load(weights_path)  # load pre-trained weights
            self.backbone.backbone.load_state_dict(efficientnet_weights, strict=False)
...
def build_SSDEfficientNet(mode, new_size, anchors, class_count):
    ...
    backbone_channels = [40, 80, 80, 112, 192, 320]
    ...
    return SSDEfficientNet(mode, backbone, [], head, anchors, class_count)
