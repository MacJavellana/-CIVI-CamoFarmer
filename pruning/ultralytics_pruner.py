...
from pruning import prune_helpers as ph


def parse_args():
    """Parse Command-Line Arguments.
    
    Parses and validates command-line arguments for the pruning script.
...
def load_model(model_type: str, model_path: str):
    ...
    Loads the specified model type and configures layer group slices for pruning.
...
def prune_model(model, prune_group: str, prune_ratio: float, group_slices: dict):
    ...
    Applies L1 unstructured pruning to specified layer groups in the model.

    Args:
        model (torch.nn.Module): The model to prune.
        prune_group (str): Layer group ('backbone', 'neck', 'head', or 'all').
        prune_ratio (float): Proportion of weights to prune.
...
def write_results_csv(results_path: str, data: list):
    ...
    Saves pruning statistics and performance metrics to a CSV file.
