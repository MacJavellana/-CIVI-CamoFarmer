...
- **ccrop_tp(txt_file, model, iou_threshold, viz_folder):** Evaluate model on file list (CCROP).  
...
def count_parameters(model):
    ...
    Counts total and non-zero parameters in a PyTorch model for sparsity evaluation.
...
def benchmark_inference(model, iterations=100, input_size=(1, 3, 320, 320)):
    ...
    Measures average inference time with GPU synchronization and warmup.

    Args:
        model (torch.nn.Module): The model to benchmark.
        iterations (int): Number of iterations for timing. Default: 100.
        input_size (tuple): Shape of dummy input (B, C, H, W). Default: (1, 3, 320, 320).
...
def compute_iou(box1, box2):
    ...
    Args:
        box1 (list or tuple): [x1, y1, x2, y2] coordinates of first box.
        box2 (list or tuple): [x1, y1, x2, y2] coordinates of second box.
...
def ccrop_tp(txt_file, model, iou_threshold, viz_folder="visualizations"):
    """Evaluate True Positives on CCROP Dataset.

    Evaluates model on CCROP dataset using text file of image paths. Creates visualizations.

    Args:
        txt_file (str): Path to .txt file listing image paths.
        model: Inference model that supports `model(image_path)` calls.
        iou_threshold (float): IoU threshold for true positive classification.
        viz_folder (str): Directory for visualization images. Default: "visualizations".
...
def dset_tp(image_dir, label_dir, model, iou_threshold, viz_folder="visualizations"):
    """Evaluate True Positives on Generic Dataset.

    Evaluates model on standard dataset with separate image/label folders. Creates visualizations.

    Args:
        image_dir (str): Directory containing test images (.jpg, .png).
        label_dir (str): Directory containing corresponding label .txt files.
        model: Inference model that supports `model(image_path)` calls.
        iou_threshold (float): IoU threshold for true positive classification.
        viz_folder (str): Directory for visualization images. Default: "visualizations".
