...
def parse_args():
    ...
    Parses and validates command-line arguments for the fine-tuning script.
...
def get_dataset_paths(dset_type: str):
    """Get Dataset Paths Based on Type.
    
    Returns dataset configuration paths for different dataset types.
...
def load_pruned_model(model_type: str, model_path: str):
    ...
    Loads a previously pruned model for fine-tuning.
    ...
    Returns:
        Model: Loaded pruned model instance.
    """
    if model_type == 'yolo':
...
def get_training_params(model_type: str, dset_type: str):
    ...
    Returns training configuration based on model and dataset type.
...
def write_results_csv(results_path: str, avg_time: float, std_time: float, ft_results, tp_percent: float):
    ...
    Saves fine-tuning performance metrics and statistics to a CSV file.
