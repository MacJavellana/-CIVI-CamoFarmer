"""
Main Pruning Pipeline for SSD.
------------------------------------------

Provides the main training and testing pipeline for pruned SSD models
with support for multiple datasets and architectures.

Functions:

- **zip_directory:** Store project files in a zip archive.
- **save_config:** Save experiment configuration.
- **string_to_boolean:** Convert string to boolean value.
- **main:** Main execution function for training/testing.
"""
...
from solver_prune import Solver
...
def zip_directory(path, zip_file):
    """Store all py and cfg project files inside a zip file.

    Args:
        path (str): Current path to scan for files.
...
def save_config(path, version, config):
    ...
    Args:
        path (str): Save path for configuration files.
        version (str): Version of the model based on timestamp.
        config (dict): Contains argument and its value.

    Returns:
        str: Version string used for file naming.
    """
...
def string_to_boolean(v):
    ...
    Args:
        v (str): String representation of a boolean value; must be 'true' or 'false'.
...
def main(version, config, output_txt):
    """Main execution function for training/testing.
    
    Args:
        version (str): Version identifier for the experiment.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prune_model', type=lambda x: str(x).lower() == 'true', default=False, help='Whether to prune the model before training')
    parser.add_argument('--prune_amount', type=float, default=0.1, help='Percentage of weights to prune')
    parser.add_argument('--prune_mode', type=str, default='all', 
                    choices=['all', 'backbone', 'neck', 'head'], 
                    help='Which part of the model to prune')
