"""
RT-DETR Knowledge Distillation Training Pipeline
-----------------------------------------------
Automated training pipeline for RT-DETR models with knowledge distillation.

Functions

- **train_model_with_fixed_hyperparameters:** Train RT-DETR with fixed distillation settings.
- **train_and_test_all_combinations:** Main pipeline for all model-dataset combinations.
- **create_final_summary:** Generate comprehensive result summaries and CSV exports.

Change directory to the location of dataset YAML files if needed
"""

from add_me.distill_model import RTDETRDistillation
...
def get_model_name(yaml_file):
    ...
    Args:
        yaml_file (str): YAML filename with extension.

    Returns:
        str: Model name without .yaml extension.
    """
...
def train_model_with_fixed_hyperparameters(model_config, dataset_config):
    """Train RT-DETR Model with Fixed Hyperparameters.

    Args:
        model_config (str): Path to model configuration YAML.
        dataset_config (dict): Dataset configuration with data path and teacher weights.

    Returns:
        list: Results for all saved epoch weights with performance metrics.
    """
...
def train_and_test_all_combinations():
    """Main Training Pipeline for All Model-Dataset Combinations.
    
    Trains all RT-DETR models with fixed hyperparameters and evaluates performance.

    Returns:
        list: Complete results for all model-dataset combinations.
    """
...
def create_final_summary(all_results):
    """Create Comprehensive Training Results Summary.

    Args:
        all_results (list): Complete training results from all experiments.

    Note:
        Generates multiple CSV files: complete results, successful only, and best per combination.
    """
