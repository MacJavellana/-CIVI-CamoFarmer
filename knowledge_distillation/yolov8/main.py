"""
YOLOv8 Knowledge Distillation Pipeline
-------------------------------------
Automated training pipeline with hyperparameter sweep and full training phases.

Functions

- **find_best_params:** Hyperparameter sweep for optimal distillation settings.
- **full_train:** Complete training with best parameters found in sweep.
- **run_pipeline:** Main orchestrator running sweep + training for all configs.
----
"""
...
# os.chdir(r"D:\path\to\yaml\files")
...
def find_best_params(model_cfg, ds_cfg):
    """Hyperparameter Sweep for Distillation.

    Args:
        model_cfg (str): Path to model configuration YAML.
        ds_cfg (dict): Dataset configuration with data path and teacher weights.

    Returns:
        dict: Best parameter combination based on mAP performance.
    """
...
def full_train(model_cfg, ds_cfg, best):
    ...
    Args:
        model_cfg (str): Path to model configuration YAML.
        ds_cfg (dict): Dataset configuration with data path and teacher weights.
        best (dict): Best hyperparameters from sweep phase.

    Returns:
        list: Results for all saved epoch weights.
    """
    LOGGER.info(
...
def run_pipeline():
    ...
    Runs hyperparameter sweep and full training for all model/dataset combinations.
    """
