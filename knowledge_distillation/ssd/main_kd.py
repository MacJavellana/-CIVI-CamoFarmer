"""
SSD Knowledge Distillation Main Training Script.
------------------------------------------
Entry point for training SSD models with knowledge distillation.
Contains comprehensive argument parsing, configuration management, and training orchestration
for teacher-student training paradigms on multiple datasets.


Functions:

    - **zip_directory:** Archive project files for experiment tracking.
    - **save_config:** Save experimental configuration and create project archive.
    - **string_to_boolean:** Convert string representations to boolean values.
    - **main:** Main training function orchestrating the knowledge distillation process.
...
def zip_directory(path, zip_file):
    """Store all Python and configuration files in a zip archive.

    Archives all .py and .cfg files from the specified path for experiment tracking
    and reproducibility purposes. Configuration files are removed after archiving.

    Args:
        path (str): Directory path to search for project files.
        zip_file (zipfile.ZipFile): Zip file object to store the project files.
    """
...
def save_config(path, version, config):
    """Save experimental configuration and create project archive.

    Saves the configuration parameters to a .cfg file and creates a zip archive
    containing all project files for experiment reproducibility and tracking.

    Args:
        path (str): Save path for configuration and archive files.
        version (str): Version identifier based on timestamp for file naming.
        config (dict): Dictionary containing all configuration parameters.

    Returns:
        str: The version string used for file naming.
    """
...
def string_to_boolean(v):
    ...
    Converts string inputs to boolean values for command-line argument parsing.

    Args:
        v (str): String representation of boolean value.

    Returns:
        bool: Boolean value corresponding to the string input.
    """
    return v.lower() in ('true')


def main(version, config, output_txt):
    """Main training function for SSD knowledge distillation.
    
    Starts the knowledge distillation training process including data loading,
    solver initialization, and training/testing execution based on the specified mode.
    Enables fast training with cudnn.benchmark optimization.
    
    Args:
        version (str): Version identifier for the training run.
        config: Configuration object containing all training parameters.
        output_txt (str): Path to output text file for logging.
    """
    # for fast training
...
if __name__ == '__main__':
    ...
    # kd
    parser.add_argument('--use_kd', type=string_to_boolean, default=False,
    ...
    parser.add_argument('--kd_warmup_epochs', type=int, default=10,
                        help='Number of epochs to apply KD before turning it off')
